  ...
  try {
    ...
    if (!collectionExists) {
      ...
      const indexFields = [
        ...
        { field: 'city', schema: 'keyword' as const },
        { field: 'startTime', schema: 'float' as const },
        { field: 'text', schema: 'text' as const }, // For BM25 search support
        { field: 'location', schema: 'geo' as const }, // For geo bounding box queries
      ...
      for (const { field, schema } of indexFields) {
        try {
          ...
          console.log(
            `Index for '${field}' might already exist:`,
            error.message,
          );
        }
      }
    } else {
      console.log(`Qdrant collection ${EVENTS_COLLECTION_NAME} already exists`);
    }
  } catch (error) {
